...
class ModelTrainer:
        ...
        try:
            ...
            for batch_idx, (data, target) in enumerate(pbar):
                ...
                pbar.set_description(
                     desc=f"Loss={loss.item()} Batch_id={batch_idx} Accuracy="
                )
        ...
        try:
            ...
            with torch.no_grad():
                for (
                    ...
                    test_loss += F.nll_loss(output, target, reuction="sum").item()
                ...
                print ("Test_set: Average_loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)\n".format
                       (test_loss, correct, len(self.data_transformation_artifact.transformed_test_object.dataset),
                        100.0*correct/len(self.data_transformation_artifacts.transformed_test_object.dataset)
                        )
                    )
                
            logging.info(
                "Test_set: Average_loss: {:.4f}, Accuarcy: {}/{} ({:.2f}%)".format
                (
                    test_loss,
                    correct,
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        try:
            logging.info(
                "Entered the initiate_model_trainer method od Model trainer class"
            )
